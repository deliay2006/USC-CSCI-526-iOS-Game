...
class GameScene2: SKScene, SKPhysicsContactDelegate {
    ...
    var explode:SKSpriteNode!//sharp rock
    ...
    override func didMoveToView(view: SKView) {
        ...
        /*------basic parts------*/
        self.hen = self.assetViewController.initHen(CGPointMake(self.screenWidth / 2, self.screenHeight - 150),size: CGSizeMake(80, 80))
        self.addChild(hen)
        ...
        self.redArrow = self.assetViewController.initArrow(CGPointMake(210, self.screenHeight - 200),size: CGSizeMake(50, 50))
        redArrow.zRotation = CGFloat(M_PI) / 2
        
        let moveUP = SKAction.moveTo(CGPointMake(210, self.screenHeight - 250), duration: 0.8)
        let moveDOWN = SKAction.moveTo(CGPointMake(210, self.screenHeight - 200), duration: 0)
        ...
        /*-------obstacles-------*/
        self.explode = self.assetViewController.initExplode("rock",position: CGPointMake(self.screenWidth / 2 , 300))
        
        self.addChild(explode)
        
                
        self.trampoline = self.assetViewController.initTrampoline("tram",position: CGPointMake(self.screenWidth - 50, 280))
        self.trampoline.zRotation = CGFloat(M_PI / 2.0)
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if(firstBody.categoryBitMask == ballCategory && secondBody.categoryBitMask == sharpRockCategory){
            explodeDidCollideWithBall(firstBody.node as! SKSpriteNode, ball: secondBody.node as! SKSpriteNode)
        }
    ...
    func basketDidCollideWithBall(basket:SKSpriteNode,ball:SKSpriteNode){
        ...
        let anotherInt2 = NSUserDefaults.standardUserDefaults().integerForKey("myInt2")
        if(myInt2<anotherInt2 || anotherInt2 == 0){
            NSUserDefaults.standardUserDefaults().setInteger(myInt2, forKey: "myInt2")
    ...
    //    self.scoreLabel.text = "Highest Score: " + self.assetViewController.scoreSystem(NSUserDefaults.standardUserDefaults().integerForKey("myInt2"))
        ...
        self.currentScoreLabel.text = "Your Score: " + self.assetViewController.scoreSystem(myInt2)
...
//        self.transitToNextScene()
    ...
    func explodeDidCollideWithBall(basket:SKSpriteNode,ball:SKSpriteNode){
        print("HIT rock")
        ...
        textures.append(SKTexture(imageNamed: "fire01"))
        
        // 3
        textures.append(SKTexture(imageNamed: "fire02"))
        textures.append(SKTexture(imageNamed: "fire03"))
        textures.append(SKTexture(imageNamed: "fire04"))
        textures.append(SKTexture(imageNamed: "fire05"))
        textures.append(SKTexture(imageNamed: "fire06"))
        textures.append(SKTexture(imageNamed: "fire07"))
        textures.append(SKTexture(imageNamed: "fire08"))
        textures.append(SKTexture(imageNamed: "fire09"))
     //   textures.append(SKTexture(imageNamed: "color_ball"))
        
        //        oneAnimation1 = SKAction.repeatActionForever( SKAction.animateWithTextures(textures,timePerFrame: 0.08))
        oneAnimation1 = SKAction.animateWithTextures(textures, timePerFrame: 0.08)
        self.ball.runAction(oneAnimation1)

        //                sharpRock.runAction(SKAction.repeatActionForever(oneAnimation))
        
        //self.ball.runAction(SKAction.repe
...
//        self.rect1.hidden = false
    ...
    func goBackGame(sender: UIButton){
        ...
        self.resumeGame()
        self.ball.position = CGPointMake(self.screenWidth / 2, screenHeight - 130)
