...
class GameScene5: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView) {
        ...
        /*------basic parts------*/
        self.hen = self.assetViewController.initHen(CGPointMake(30, 500),size: CGSizeMake(80, 80))
        self.addChild(hen)
        
        self.ball = self.assetViewController.initBall(CGPointMake(70, 490),size: CGSizeMake(30, 30))
        self.addChild(ball)
...
//        self.addChild(self.windBlower)
        ...
        self.bubble = self.assetViewController.initBubble("bubble", position: CGPointMake(80, 200))
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if let body = self.physicsWorld.bodyAtPoint(location){
            ...
            if body.node!.name == "bubble" {
                print("bubble touched")
                self.bubble.removeFromParent()
                self.ball.alpha = 1.0
//                self.ball.removeActionForKey("float")
                self.ball.removeAllActions()
                //self.bubble.texture = SKTexture.init(imageNamed: "round");
                //need do something to add the ball again and delete bubble
            }
    ...
    func bubbleDidCollideWithBall(ball:SKSpriteNode,bubble:SKSpriteNode) {
        print("bubble collide with ball")
        
        //ball.removeFromParent()
        bubble.texture = SKTexture.init(imageNamed: "bubble_with_ball")
        
//        let screenSize:CGSize = self.frame.size;
        let bubbleVelocity = self.screenHeight / bubble.size.height;
        let location:CGPoint = CGPointMake(bubble.position.x, self.screenHeight);
      
        let moveAction:SKAction = SKAction.moveTo(location, duration: NSTimeInterval.init(bubbleVelocity));

        bubble.runAction(moveAction, withKey: "float")
        ball.alpha = 0.0
        let ballMoveAction:SKAction = SKAction.moveTo(CGPointMake(ball.position.x, self.screenHeight), duration: NSTimeInterval.init(bubbleVelocity))
        ball.runAction(ballMoveAction, withKey: "float")
    }
    ...
    func goBackGame(sender: UIButton){
       ...
       // self.assetViewController.initBubble("bubble", position: CGPointMake(<#T##x: CGFloat##CGFloat#>, <#T##y: CGFloat##CGFloat#>))
